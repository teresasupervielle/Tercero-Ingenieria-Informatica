...
static inline bool read_input(void){
	static bool flag					= true;
	bool button_CurrentState			= true;
	
	button_CurrentState = READ_BUTTON(GPIO_D_IN, BUTTON_PIN);
	...
	button_CurrentState = READ_BUTTON(GPIO_D_IN,BUTTON_PIN); // Second reading
	
	if ((button_CurrentState==BUTTON_PRESSED) && (flag==BUTTON_NOT_PRESSED)){	// FAlling-edge
		_delay_ms(20); // Debouncing time
		button_CurrentState = READ_BUTTON(GPIO_D_IN,BUTTON_PIN); // Third reading
