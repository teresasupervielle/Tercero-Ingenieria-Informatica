...
int32_t leer_joystick(void)
{
    ...
    if(!(LPC_GPIO2->PIN & (1<<25)))	//Leemos pin -> si 0 en el puerto 25-> jostick llevado hacia arriba 
			return JOYSTICK_ARRIBA;
		
		if(!(LPC_GPIO2->PIN & (1<<27)))  //Leemos pin -> si 0 en el puerto 27-> jostick llevado hacia abajo 
			return JOYSTICK_ABAJO;
		
		if(!(LPC_GPIO2->PIN & (1<<23))) 	//Leemos pin -> si 0 en el puerto 23-> jostick llevado hacia izq 
			return JOYSTICK_IZQUIERDA;
		
		if(!(LPC_GPIO2->PIN & (1<<26)))		//Leemos pin -> si 0 en el puerto 26-> jostick llevado hacia der 
			return JOYSTICK_DERECHA;
		
		if(!(LPC_GPIO2->PIN & (1<<22)))	 //Leemos pin -> si 0 en el puerto 22-> jostick pulsado && encendemos leds
		{	
			led_encender(LED1);
			led_encender(LED2);
			led_encender(LED3);
