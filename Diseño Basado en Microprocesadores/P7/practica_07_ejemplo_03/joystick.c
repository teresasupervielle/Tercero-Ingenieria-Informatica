...
{
    if(!(gpio_leer_pin(PUERTO2, PIN25)))	//Leemos pin -> si 1 en el puerto 25-> jostick llevado hacia arriba 
			return JOYSTICK_ARRIBA;
		
		if(!(gpio_leer_pin(PUERTO2, PIN27)))  //Leemos pin -> si 1 en el puerto 27-> jostick llevado hacia abajo 
			return JOYSTICK_ABAJO;
		
		if(!(gpio_leer_pin(PUERTO2, PIN23))) 	//Leemos pin -> si 1 en el puerto 23-> jostick llevado hacia izq 
			return JOYSTICK_IZQUIERDA;
		
		if(!(gpio_leer_pin(PUERTO2, PIN26)))		//Leemos pin -> si 1 en el puerto 26-> jostick llevado hacia der 
			return JOYSTICK_DERECHA;
		
		if(!(gpio_leer_pin(PUERTO2, PIN22)))	 //Leemos pin -> si 1 en el puerto 22-> jostick pulsado && encendemos leds
		{	
			led_encender(LED1);
			led_encender(LED2);
			led_encender(LED3);
