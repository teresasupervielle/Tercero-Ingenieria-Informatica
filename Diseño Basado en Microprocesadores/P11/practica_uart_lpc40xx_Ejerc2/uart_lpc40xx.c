...
void uart_transmitir_cadena(LPC_UART_TypeDef *uart_regs, const char *cadena)
{
    ASSERT(uart_regs == UART0 || uart_regs == UART1 || uart_regs == UART2 ||
		...
		while(*cadena != 0){
			uart_transmitir_dato (uart_regs, *cadena);
			cadena++;
		}
}
                         ...
                         uint32_t tamano_buffer)
{
    char caracter_recibido;
		...
		while (TRUE){
			caracter_recibido = uart_esperar_recibir_dato (uart_regs);
			if (caracter_recibido >= 0x20 && tamano_buffer > 1){
				*ptr_buffer = caracter_recibido;
				tamano_buffer--;
				ptr_buffer++;
			}
			if (caracter_recibido == 0x0A){
				*ptr_buffer = 0;
				return;
			}
		}
