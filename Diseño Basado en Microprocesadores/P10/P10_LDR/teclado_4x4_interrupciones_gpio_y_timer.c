/***************************************************************************//**
 * \file    teclado_4x4_interrupciones_gpio_y_timer.c
 *
 ...
 *          Fila 3      *           0           #           D
 *
 *          Esta versión usa interrupciones GPIO a través de los pines de
 *          columna para disparar el barrido del teclado sólo cuando se realiza
 *          una pulsación en lugar de realizar el barrido continuamente como
 *          la versión teclado_4x4_interrupcion_timer.c.
 *
 *          Se incluye un buffer para guardar pulsaciones hasta que la
 ...
 *
 *          Los cuatro pines del microcontrolador que conectan con las filas
 *          quedan configuradas como salida y se ponen a 0.
 *
 *          Los cuatro pines del microcontrolador que conectan con las columnas
 *          quedan configuradas como entradas y permanecerán así todo el tiempo
 *          (ver nota). Los pines de columna deben pertenecer al puerto 0 o 2,
 *          para que tengan capacidad de generar interrupciones.
 *          
 *          Se inicializa el timer usado por las funciones del teclado y se
 *          prepara para que realice ciclos de duración TEC4X4_TIEMPO_FILA_US
 *          (definido en teclado_4x4.h) pero no se pone en marcha.
 *
 ...
 *
 *          Se habilitan interrupciones por flanco de bajada en los pines de
 *          columna.
 *
 ...
 *
 *          Se programa el timer seleccionado para generar una interrupción
 *          periódica cada TEC4X4_TIEMPO_FILA_US microsegundos.
 */
void tec4x4_inicializar(void)
{
    ...
    /* Inicialmente, todos los pines conectados a las filas se consideran como
     * salidas y se ponen a 0.
     */
    gpio_ajustar_dir(TEC4X4_PUERTO_FILAS,
                     TEC4X4_MASCARA_TODAS_LAS_FILAS,
                     DIR_SALIDA);
    gpio_pin_a_0(TEC4X4_PUERTO_FILAS, TEC4X4_MASCARA_TODAS_LAS_FILAS);
    ...
    /* Inicializar el timer usado por el teclado y configurarlo para generar ciclos
     * de periodo TEC4X4_TIEMPO_FILA_US pero mantenerlo detenido por el momento.
     */
    timer_inicializar(TEC4X4_TIMER);    
    TEC4X4_TIMER->TCR = 0;
    TEC4X4_TIMER->PC = 0;
    TEC4X4_TIMER->TC = 0;
    TEC4X4_TIMER->PR = PeripheralClock/1000000 - 1;
    TEC4X4_TIMER->MR0 = TEC4X4_TIEMPO_FILA_US - 1;
    TEC4X4_TIMER->MCR = 3;
    TEC4X4_TIMER->IR = 1;
    ...
    /* Configurar y habilitar las interrupciones a través de los pines conectados
     * a las columnas.
     */
    LPC_GPIOINT->TEC4X4_CLR = TEC4X4_MASCARA_TODAS_LAS_COLUMNAS;
    LPC_GPIOINT->TEC4X4_ENF = TEC4X4_MASCARA_TODAS_LAS_COLUMNAS;
    NVIC_ClearPendingIRQ(GPIO_IRQn);
    NVIC_SetPriority(GPIO_IRQn, TEC4X4_PRIORIDAD_INTERRUPCION);
...
/***************************************************************************//**
 * \brief   Manejador de interrupción de las interrupciones a través de los
 *          pines de los puertos 0 y 2.
 */
void GPIO_IRQHandler(void)
{
    /* Si la interrupción la causó alguno de los pines de columna del
     * teclado ...
     */
    if ((LPC_GPIOINT->TEC4X4_STATF & TEC4X4_MASCARA_TODAS_LAS_COLUMNAS) != 0)
    {
        /* Configurar como entradas los pines de fila 1 a 3 y dejar como
         * salida y a 0 únicamente la fila 0.
         */
        gpio_ajustar_dir(TEC4X4_PUERTO_FILAS,
                         TEC4X4_MASCARA_FILA_1 |
                         TEC4X4_MASCARA_FILA_2 |
        ...
        /* Deshabilitar interrupciones por los pines de columna hasta
         * que se complete la lectura del teclado en el manejador de
         * interrupción del timer.
         */
        LPC_GPIOINT->TEC4X4_ENF &= ~TEC4X4_MASCARA_TODAS_LAS_COLUMNAS;
        LPC_GPIOINT->TEC4X4_CLR = TEC4X4_MASCARA_TODAS_LAS_COLUMNAS;
        
        /* Poner a cero los registros contadores del timer y ponerlo en
         * marcha. El timer generará una interrupción dentro de
         * TEC4X4_TIEMPO_FILA_US microsegundos.
         */
        TEC4X4_TIMER->PC = 0;
        TEC4X4_TIMER->TC = 0;
        TEC4X4_TIMER->TCR = 1;            
    ...
    /* Otras interrupciones de GPIO pueden procesarse aquí.
     */
}
    ...
    {        
        ...
        if (estado_ultimo_barrido == estado_ultimo_cambio)
        {
            ...
            {                
                 ...
                 * una "no pulsación", introducir el carácter en el buffer de
                 * teclado.
                 */
                ...
                /* Se vuelve a configurar todos los pines de fila como salidas
                 * y se ponen a 0.
                 */
                gpio_ajustar_dir(TEC4X4_PUERTO_FILAS,
                                 TEC4X4_MASCARA_TODAS_LAS_FILAS,
                ...
                /* Se para el timer para detener la generación de sus interrupciones
                 * hasta que el manejador de interrupciones GPIO lo ponga de nuevo en
                 * marcha.
                 */
                TEC4X4_TIMER->TCR = 0;
                TEC4X4_TIMER->IR = 1;
                
                /* Se vuelven a habilitar interrupciones por flanco de bajada en
                 * todos los pines de columna.
                 */
                LPC_GPIOINT->TEC4X4_CLR = TEC4X4_MASCARA_TODAS_LAS_COLUMNAS;
                LPC_GPIOINT->TEC4X4_ENF |= TEC4X4_MASCARA_TODAS_LAS_COLUMNAS; 
            }      
