...
/*===========================================================================
 * La constante ALINEAMIENTO determina el alineamiento del buffer que se
 * ubicará para la imagen. Pueden darse valores entre 0 y 32.
 */
#define ALINEAMIENTO 32
...
BMP* BMP_ReadFile( const char* filename )
{
	...
	/*=======================================================================
	 * Modificaciones para poder alterar el alineamiento del buffer ubicado
	 * para la imagen y así poner de manifiesto la importancia de que los
	 * datos estén alineados en la memoria para obtener las máximas
	 * prestaciones de las instrucciones MMX/SSE/AVX.
	 */

	/* Ubicar un buffer 32 bytes más grande de lo necesario.
	 */

	bmp->Data_ubicado = (UCHAR*) malloc( bmp->Header.ImageDataSize + 32 );

	if ( bmp->Data_ubicado == NULL )
	{
		BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
		fclose( f );
		free( bmp->Palette );
	...
	/* Transformar el puntero a un entero para poderlo manipular
	 */
	unsigned long temp = (unsigned long)bmp->Data_ubicado;

	/* Redondear al entero divisible entre 32 más cercano
	 * (él mismo o el siguiente divisible entre 32 por arriba).
	 */

	temp = (temp + 31) & (~31);

	/* Avanzar el puntero en la cantidad indicada por ALINEAMIENTO.
	 */

	temp += ALINEAMIENTO;

	/* Transformar de vuelta el entero en un puntero.
	 */

	bmp->Data = (unsigned char *)temp;
